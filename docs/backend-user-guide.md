...
- `NucleariesInput`
  - `title`, `content`, `description`, `operation`, `constants`
...
##### Category and Query-Surface MVP (Current)

Crystord keeps the MVP category model intentionally small and deterministic.

- Existing `labels` remain supported as lightweight tags for retrieval compatibility.
- Category nodes use the system-reserved `Category` label.
...
- The required Neo4j index/constraint baseline is created at runtime so category and label queries stay stable.
...
- MVP callable names are: `SUM`, `MINUS`, `PRODUCT`, `DIVIDE`.
